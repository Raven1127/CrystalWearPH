...
function getProd()
{
    ...
    <div class='col-lg-6'>
        <div class='product-details'>
            ...
            <ul class='pd-tags'>
                <li><span>Stocks:</span> $stock</li>
            </ul>
        <input type='hidden' id='stck' value='".$stock."'>
        ";
        if($stock == 0){
            echo "<script>alert('No stocks available!');
...
function cart_items()
{
    ...
    <div class='card col-md-3 col-10' style='margin:0 auto; border-radius:25px 5px;box-shadow: inset -12px -8px 40px #e5e5e5;'>
        ...
        while ($row_items = mysqli_fetch_array($run_itemss)) {
            ...
            echo "
        <form method='post' action='shopping-cart.php?upd=$pro_id'>
        <tr style='border-bottom: 0.5px solid #ebebeb'>
           ...
           <td class='qua-col first-row'>$pro_size</td>
           <td class='p-price first-row'>PHP $pro_price</td>
           <td class='qua-col first-row'>
               <div class='quantity'>
                   <div class='pro-qty'>
                       <input id = 'qqty".$pro_id."' type='number' name='quant' value='$pro_qty'>
                   </div>
               </div>
           </td>
           <input type='text' id='trcv".$pro_id."' name='quantx".$pro_id."' style='display:none;'>
           <td class='total-price first-row'>PHP $pro_total_p</td>
           <td class='close-td first-row'><a href='shopping-cart.php?del=$pro_id'><i class='ti-close' style='color:black'></i></a></td>
           <td class='close-td first-row'><button onclick='yuu".$pro_id."()' class='primary-btn up-cart' style='border:none;' type='submit' value='Update'>Update</button></td>
       </tr>    
       </form>
       <script>
       function yuu".$pro_id."(){
        document.getElementById('trcv".$pro_id."').value = document.getElementById('qqty".$pro_id."').value;
       }
       </script>
